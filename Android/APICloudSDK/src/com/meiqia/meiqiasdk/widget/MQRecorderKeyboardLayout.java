...
public class MQRecorderKeyboardLayout extends MQBaseCustomCompositeView implements MQAudioRecorderManager.Callback, View.OnTouchListener {
    ...
    protected int getLayoutId() {
        return MQResUtils.getResLayoutID("mq_layout_recorder_keyboard");
    }

    @Override
    protected void initView() {
        mStatusTv = getViewById(MQResUtils.getResIdID("tv_recorder_keyboard_status"));
        mAnimIv = getViewById(MQResUtils.getResIdID("iv_recorder_keyboard_anim"));
    }
    ...
    private void initLevelListDrawable() {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            levelListDrawable.addLevel(i, i + 1, MQUtils.tintDrawable(getContext(), getResources().getDrawable(resId), MQResUtils.getResColorID("mq_chat_audio_recorder_icon")));
        }
        levelListDrawable.addLevel(9, 10, getResources().getDrawable(MQResUtils.getResDrawableID("mq_voice_want_cancel")));
        ...
        post(new Runnable() {
            ...
            public void run() {
                if (mCurrentState == STATE_RECORDING) {
                    ...
                    if (remainingTime <= 10) {
                        mStatusTv.setText(getContext().getString(MQResUtils.getResStringID("mq_recorder_remaining_time"), remainingTime));
                    }
                }
            }
        });
    ...
    private void changeState(int status) {
        if (mCurrentState != status) {
            ...
            switch (mCurrentState) {
                case STATE_NORMAL:
                    mStatusTv.setText(MQResUtils.getResStringID("mq_audio_status_normal"));
                    mAnimIv.setImageLevel(1);
                    break;
                case STATE_RECORDING:
                    mStatusTv.setText(MQResUtils.getResStringID("mq_audio_status_recording"));
                    break;
                case STATE_WANT_CANCEL:
                    mStatusTv.setText(MQResUtils.getResStringID("mq_audio_status_want_cancel"));
