...
public class MQPhotoFolderPw extends MQBasePopupWindow implements AdapterView.OnItemClickListener {
    ...
    public MQPhotoFolderPw(Activity activity, View anchorView, Callback callback) {
        super(activity, MQResUtils.getResLayoutID("mq_pw_photo_folder"), anchorView, WindowManager.LayoutParams.MATCH_PARENT, WindowManager.LayoutParams.MATCH_PARENT);
        mCallback = callback;
    }

    @Override
    protected void initView() {
        mRootLl = getViewById(MQResUtils.getResIdID("root_ll"));
        mContentLv = getViewById(MQResUtils.getResIdID("content_lv"));
    }
    ...
    private class FolderAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                convertView = LayoutInflater.from(parent.getContext()).inflate(MQResUtils.getResLayoutID("mq_item_photo_folder"), parent, false);
                folderViewHolder = new FolderViewHolder();
                folderViewHolder.photoIv = (MQImageView) convertView.findViewById(MQResUtils.getResIdID("photo_iv"));
                folderViewHolder.nameTv = (TextView) convertView.findViewById(MQResUtils.getResIdID("name_tv"));
                folderViewHolder.countTv = (TextView) convertView.findViewById(MQResUtils.getResIdID("count_tv"));
            ...
            MQImage.displayImage(mActivity, folderViewHolder.photoIv, imageFolderModel.coverPath, MQResUtils.getResDrawableID("mq_ic_holder_light"), MQResUtils.getResDrawableID("mq_ic_holder_light"), mImageWidth, mImageHeight, null);
