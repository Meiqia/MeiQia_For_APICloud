...
public abstract class MQBaseBubbleItem extends MQBaseCustomCompositeView implements MQChatFileItem.Callback {
    ...
    protected void initView() {
        contentText = getViewById(MQResUtils.getResIdID("content_text"));
        contentImage = getViewById(MQResUtils.getResIdID("content_pic"));
        voiceContentTv = getViewById(MQResUtils.getResIdID("tv_voice_content"));
        voiceAnimIv = getViewById(MQResUtils.getResIdID("iv_voice_anim"));
        voiceContainerRl = getViewById(MQResUtils.getResIdID("rl_voice_container"));
        chatFileItem = getViewById(MQResUtils.getResIdID("file_container"));
        usAvatar = getViewById(MQResUtils.getResIdID("us_avatar_iv"));
        chatBox = getViewById(MQResUtils.getResIdID("chat_box"));
    }
    ...
    private void configChatBubbleBg(View view, boolean isLeft) {
        if (isLeft) {
            MQUtils.applyCustomUITintDrawable(view, MQResUtils.getResColorID("mq_chat_left_bubble_final"), MQResUtils.getResColorID("mq_chat_left_bubble"), MQConfig.ui.leftChatBubbleColorResId);
        } else {
            MQUtils.applyCustomUITintDrawable(view, MQResUtils.getResColorID("mq_chat_right_bubble_final"), MQResUtils.getResColorID("mq_chat_right_bubble"), MQConfig.ui.rightChatBubbleColorResId);
        }
    }
    ...
    private void configChatBubbleTextColor(TextView textView, boolean isLeft) {
        if (isLeft) {
            ...
            MQUtils.applyCustomUITextAndImageColor(MQResUtils.getResColorID("mq_chat_right_textColor"), MQConfig.ui.rightChatTextColorResId, null, textView);
        }
    }
    ...
    private void fillContent(BaseMessage baseMessage, final int position, Activity activity) {
        ...
        switch (baseMessage.getContentType()) {
            ...
            case BaseMessage.TYPE_CONTENT_PHOTO:
                ...
                MQImage.displayImage(activity, contentImage, url, MQResUtils.getResDrawableID("mq_ic_holder_light"), MQResUtils.getResDrawableID("mq_ic_holder_light"), mImageWidth, mImageHeight, new MQImageLoader.MQDisplayImageListener() {
                    @Override
            ...
            // 默认文字消息处理
            default:
                contentText.setText(getResources().getString(MQResUtils.getResStringID("mq_unknown_msg_tip")));
                break;
        }
    }
    ...
    private void handleBindVoiceItem(final VoiceMessage voiceMessage, final int position) {
        ...
        if (mCallback.getCurrentPlayingItemPosition() != position) {
            if (voiceMessage.getItemViewType() == BaseMessage.TYPE_AGENT) {
                voiceAnimIv.setImageResource(MQResUtils.getResDrawableID("mq_voice_left_normal"));
                voiceAnimIv.setColorFilter(getResources().getColor(MQResUtils.getResColorID("mq_chat_left_textColor")));
            } else {
                voiceAnimIv.setImageResource(MQResUtils.getResDrawableID("mq_voice_right_normal"));
                voiceAnimIv.setColorFilter(getResources().getColor(MQResUtils.getResColorID("mq_chat_right_textColor")));
            }
        } else {
            if (voiceMessage.getItemViewType() == BaseMessage.TYPE_AGENT) {
                voiceAnimIv.setImageResource(MQResUtils.getResDrawableID("mq_anim_voice_left_playing"));
            } else {
                voiceAnimIv.setImageResource(MQResUtils.getResDrawableID("mq_anim_voice_right_playing"));
            }
    ...
    private void downloadAndPlayVoice(final VoiceMessage voiceMessage, final int position) {
        ...
        MQDownloadManager.getInstance(getContext()).downloadVoice(voiceMessage.getUrl(), new MQDownloadManager.Callback() {
            ...
            public void onFailure() {
                MQUtils.showSafe(getContext(), MQResUtils.getResStringID("mq_download_audio_failure"));
            }
        });
    }
