...
public class MQTipItem extends MQBaseCustomCompositeView {
    ...
    protected int getLayoutId() {
        return MQResUtils.getResLayoutID("mq_item_msg_tip");
    }
    ...
    private void setDirectionMessageContent(String agentNickName) {
        if (agentNickName != null) {
            String text = String.format(getResources().getString(MQResUtils.getResStringID("mq_direct_content")), agentNickName);
            int start = text.indexOf(agentNickName);
            SpannableStringBuilder style = new SpannableStringBuilder(text);
            style.setSpan(new ForegroundColorSpan(getResources().getColor(MQResUtils.getResColorID("mq_chat_direct_agent_nickname_textColor"))), start, start + agentNickName.length(), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
