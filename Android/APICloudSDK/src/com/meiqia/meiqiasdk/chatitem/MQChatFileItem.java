...
public class MQChatFileItem extends MQBaseCustomCompositeView implements View.OnTouchListener {
    ...
    protected int getLayoutId() {
        return MQResUtils.getResLayoutID("mq_item_file_layout");
    }

    @Override
    protected void initView() {
        root = findViewById(MQResUtils.getResIdID("root"));
        mProgressBar = (CircularProgressBar) findViewById(MQResUtils.getResIdID("progressbar"));
        mTitleTv = (TextView) findViewById(MQResUtils.getResIdID("mq_file_title_tv"));
        mSubTitleTv = (TextView) findViewById(MQResUtils.getResIdID("mq_file_sub_title_tv"));
        mRightIv = findViewById(MQResUtils.getResIdID("mq_right_iv"));
    }
    ...
    public void onClick(View view) {
        ...
        int id = view.getId();
        if (id == MQResUtils.getResIdID("mq_right_iv")) {
            root.performClick();
        } else if (id == MQResUtils.getResIdID("progressbar")) {
            cancelDownloading();
        } else if (id == MQResUtils.getResIdID("root")) {
    ...
    private void openFile() {
        ...
        try {
            ...
            Toast.makeText(getContext(), MQResUtils.getResStringID("mq_no_app_open_file"), Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void downloadingState() {
        mSubTitleTv.setText(String.format("%s%s", getSubTitlePrefix(), getResources().getString(MQResUtils.getResStringID("mq_downloading"))));
        mRightIv.setVisibility(GONE);
    ...
    private void displayFileInfo() {
        ...
        if (isFileExist) {
            endStr = getResources().getString(MQResUtils.getResStringID("mq_download_complete"));
            mRightIv.setVisibility(GONE);
        } else {
            ...
            if (diffTime <= 0) {
                endStr = getResources().getString(MQResUtils.getResStringID("mq_expired"));
                mRightIv.setVisibility(GONE);
                ...
                endStr = getContext().getString(MQResUtils.getResStringID("mq_expire_after"), leaveHoursStr);
