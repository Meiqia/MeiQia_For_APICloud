...
public class MQCollectInfoActivity extends MQBaseActivity implements View.OnClickListener {
    ...
    protected void initView(Bundle savedInstanceState) {
        mLoadingProgressBar = (ProgressBar) findViewById(MQResUtils.getResIdID("progressbar"));
        mSubmitTv = (TextView) findViewById(MQResUtils.getResIdID("submit_tv"));
        mContainerLl = (LinearLayout) findViewById(MQResUtils.getResIdID("container_ll"));
        mRootView = (RelativeLayout) findViewById(MQResUtils.getResIdID("root"));
        mBodyRl = (RelativeLayout) findViewById(MQResUtils.getResIdID("body_rl"));
        mScrollView = findViewById(MQResUtils.getResIdID("content_sv"));
    }
    ...
    private void hookField(String field_name, JSONObject field) {
        ...
        if ("gender".equals(field_name)) {
            try {
                ...
                String genderChoice = getResources().getString(MQResUtils.getResStringID("mq_inquire_gender_choice"));
    ...
    public void popInvalidTip() {
        if (mTopTipViewTv == null) {
            ...
            mTopTipViewTv.setText(MQResUtils.getResStringID("mq_tip_required_before_submit"));
            mTopTipViewTv.setBackgroundColor(getResources().getColor(MQResUtils.getResColorID("mq_error")));
            int height = getResources().getDimensionPixelOffset(MQResUtils.getResDimenID("mq_top_tip_height"));
            RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, height);
            params.addRule(RelativeLayout.ALIGN_TOP, MQResUtils.getResIdID("content_sv"));
            // 这里写死了位置,如果改了布局要注意
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == MQResUtils.getResIdID("submit_tv")) {
            ...
            MQManager.getInstance(this).submitInquireForm(clientIdOrCustomizedId, params, headers, new SimpleCallback() {
                ...
                public void onFailure(int code, String message) {
                    ...
                    if (code == 400) {
                        ...
                        Toast.makeText(MQCollectInfoActivity.this, MQResUtils.getResStringID("mq_error_auth_code_wrong"), Toast.LENGTH_SHORT).show();
                    } else if (code == ErrorCode.NET_NOT_WORK) {
                        Toast.makeText(MQCollectInfoActivity.this, MQResUtils.getResStringID("mq_title_net_not_work"), Toast.LENGTH_SHORT).show();
                    } else {
                        Toast.makeText(MQCollectInfoActivity.this, MQResUtils.getResStringID("mq_error_submit_form"), Toast.LENGTH_SHORT).show();
                    }
                }
            });
        } else {
    ...
    private abstract class BaseItem {
        ...
        public void initTitle() {
            ...
            if (!optional) {
                ...
                tipSsb.setSpan(new ForegroundColorSpan(getResources().getColor(MQResUtils.getResColorID("mq_error"))), titleTv.getText().length() + 1, tipSsb.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        ...
        protected void validState() {
            titleTv.setTextColor(getResources().getColor(MQResUtils.getResColorID("mq_form_tip_textColor")));
        }

        protected void invalidState() {
            titleTv.setTextColor(getResources().getColor(MQResUtils.getResColorID("mq_error")));
        }
    ...
    private class TextItem extends BaseItem {
        ...
        @Override
        void findViews() {
            rootView = getLayoutInflater().inflate(MQResUtils.getResLayoutID("mq_item_form_type_text"), null);
            titleTv = (TextView) rootView.findViewById(MQResUtils.getResIdID("title_tv"));
            contentEt = (EditText) rootView.findViewById(MQResUtils.getResIdID("content_et"));
        }
    ...
    private class SingleChoiceItem extends BaseItem implements CompoundButton.OnCheckedChangeListener {
        ...
        private void initData() {
            try {
                ...
                for (int i = 0; i < choiceArray.length(); i++) {
                    RadioButton radioButton = (RadioButton) getLayoutInflater().inflate(MQResUtils.getResLayoutID("mq_item_form_radio_btn"), null);
                    radioButton.setText(choiceArray.getString(i));
                    radioButton.setTag(choiceArray.get(i));
                    radioButton.setId(View.NO_ID);
                    radioButton.setOnCheckedChangeListener(this);
                    MQUtils.tintCompoundButton(radioButton, MQResUtils.getResDrawableID("mq_radio_btn_uncheck"), MQResUtils.getResDrawableID("mq_radio_btn_checked"));
        ...
        @Override
        void findViews() {
            rootView = getLayoutInflater().inflate(MQResUtils.getResLayoutID("mq_item_form_type_single_choice"), null);
            titleTv = (TextView) rootView.findViewById(MQResUtils.getResIdID("title_tv"));
            radioGroup = (RadioGroup) rootView.findViewById(MQResUtils.getResIdID("radio_group"));
        }
    ...
    private class MultipleChoiceItem extends BaseItem implements CompoundButton.OnCheckedChangeListener {
        ...
        private void initData() {
            try {
                JSONArray choiceArray = new JSONArray(choices);
                for (int i = 0; i < choiceArray.length(); i++) {
                    CheckBox checkBox = (CheckBox) getLayoutInflater().inflate(MQResUtils.getResLayoutID("mq_item_form_checkbox"), null);
                    checkBox.setText(choiceArray.getString(i));
                    checkBox.setOnCheckedChangeListener(this);
                    checkBox.setTag(choiceArray.get(i));
                    MQUtils.tintCompoundButton(checkBox, MQResUtils.getResDrawableID("mq_checkbox_uncheck"), MQResUtils.getResDrawableID("mq_checkbox_unchecked"));
        ...
        @Override
        void findViews() {
            rootView = getLayoutInflater().inflate(MQResUtils.getResLayoutID("mq_item_form_type_multiple_choice"), null);
            titleTv = (TextView) rootView.findViewById(MQResUtils.getResIdID("title_tv"));
            checkboxContainer = (LinearLayout) rootView.findViewById(MQResUtils.getResIdID("checkbox_container"));
        }
    ...
    private class CodeAuthItem extends BaseItem {
        ...
        @Override
        void findViews() {
            rootView = getLayoutInflater().inflate(MQResUtils.getResLayoutID("mq_item_form_type_auth_code"), null);
            titleTv = (TextView) rootView.findViewById(MQResUtils.getResIdID("title_tv"));
            authCodeEt = (EditText) rootView.findViewById(MQResUtils.getResIdID("auth_code_et"));
            authCodeIv = (ImageView) rootView.findViewById(MQResUtils.getResIdID("auth_code_iv"));
        }
                ...
                public void run() {
                    try {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                MQImage.displayImage(MQCollectInfoActivity.this, authCodeIv, captcha_image, MQResUtils.getResDrawableID("mq_ic_holder_avatar"), MQResUtils.getResDrawableID("mq_ic_holder_avatar"), authCodeIv.getWidth(), authCodeIv.getHeight(), null);
